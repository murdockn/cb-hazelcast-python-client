...
class PortableSerializer(StreamSerializer):
    ...
    def create_new_portable_instance(self, factory_id, class_id):
        try:
            ...
            raise HazelcastSerializationError("Could not find portable_factory for factory-id: {0}".format(factory_id))

        portable = portable_factory[class_id]
        if portable is None:
            raise HazelcastSerializationError("Could not create Portable for class-id: {0}".format(class_id))
        return portable()
